import { getReceivablesAging } from '@/lib/actions/receivables-actions';
...
export default async function ReceivablesAgingPage() {
  ...
  const totals = agingData.reduce((acc: any, row: any) => {
    acc.totalReceivable += row.totalReceivable;
    acc.current += row.current;
    acc.days31To60 += row.days31To60;
    acc.days61To90 += row.days61To90;
    acc.over90 += row.over90;
    return acc;
  }, { totalReceivable: 0, current: 0, days31To60: 0, days61To90: 0, over90: 0 });

  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Accounts Receivable Aging</h1>
        <p className="text-muted-foreground">Detailed breakdown of outstanding customer debts by age</p>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="text-right border-l font-bold bg-muted/20">Total Receivable</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {agingData.map((row: any) => (
                <TableRow key={row.customerId}>
                  <TableCell className="font-medium">
                    <Link href={`/admin/customers/${row.customerId}`} className="hover:underline text-primary">
                      {row.customerName}
                    </Link>
                  </TableCell>
                  ...
                  <TableCell className="text-right border-l font-bold bg-emerald-500/10 text-emerald-700">
                    ৳{row.totalReceivable.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </TableCell>
                </TableRow>
              ))}
              ...
              {agingData.length > 0 ? (
                <TableRow className="bg-muted/50 font-bold border-t-2">
                  ...
                  <TableCell className="text-right border-l text-lg text-emerald-700">
                    ৳{totals.totalReceivable.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </TableCell>
                </TableRow>
              ) : (
                <TableRow>
