...
export function ProductSEOSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Advanced AI Data (Hidden by default, shown if data exists) */}
      <div className="pt-4 border-t space-y-4">
        <h4 className="text-sm font-medium">Advanced AI Generated Data</h4>
        <p className="text-xs text-muted-foreground">This data is generated by the 3-Step AI Pipeline. It includes Schema.org, OpenGraph, Category suggestions, and Related Entities.</p>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label className="text-xs font-medium">Advanced SEO (JSON)</Label>
            <Textarea 
              className="font-mono text-[11px] h-48 bg-muted/30"
              placeholder="{}"
              value={JSON.stringify(watch('advancedSeo') || {}, null, 2)}
              onChange={(e) => {
                try {
                  const parsed = JSON.parse(e.target.value);
                  setValue('advancedSeo', parsed, { shouldDirty: true });
                } catch(e) {}
              }}
            />
          </div>
          ...
          <div className="space-y-1.5">
            <Label className="text-xs font-medium">Enrichment Data (JSON)</Label>
            <Textarea 
              className="font-mono text-[11px] h-48 bg-muted/30"
              placeholder="{}"
              value={JSON.stringify(watch('enrichmentData') || {}, null, 2)}
              onChange={(e) => {
                try {
                  const parsed = JSON.parse(e.target.value);
                  setValue('enrichmentData', parsed, { shouldDirty: true });
                } catch(e) {}
              }}
            />
          </div>
