      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col p-0 overflow-hidden border-0 shadow-2xl rounded-2xl">
        <DialogHeader className="px-6 py-5 border-b border-gray-100 shrink-0 bg-white pr-14 relative">
          ...
          <div className="flex items-center gap-4 sm:gap-8 mt-5">
            <div className="flex items-center gap-3 bg-blue-50/50 px-4 py-2.5 rounded-xl border border-blue-100/50 flex-1">
              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center shrink-0">
                <ShoppingCart className="h-5 w-5 text-blue-600" />
              </div>
              <div>
                <p className="text-[11px] text-blue-600/80 font-bold uppercase tracking-wider">Orders</p>
                <p className="text-lg font-black text-blue-900 leading-none mt-0.5">{orders.length}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 bg-purple-50/50 px-4 py-2.5 rounded-xl border border-purple-100/50 flex-1">
              <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center shrink-0">
                <Package className="h-5 w-5 text-purple-600" />
              </div>
              <div>
                <p className="text-[11px] text-purple-600/80 font-bold uppercase tracking-wider">Items Sold</p>
                <p className="text-lg font-black text-purple-900 leading-none mt-0.5">{totalItems}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 bg-green-50/50 px-4 py-2.5 rounded-xl border border-green-100/50 flex-1">
              <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center shrink-0">
                <DollarSign className="h-5 w-5 text-green-600" />
              </div>
              <div>
                <p className="text-[11px] text-green-600/80 font-bold uppercase tracking-wider">Revenue</p>
                <p className="text-lg font-black text-green-900 leading-none mt-0.5">৳{totalSales.toLocaleString()}</p>
              </div>
            </div>
          </div>
        </DialogHeader>
              ...
              {orders.map((order) => {
                ...
                return (
                  <div key={order.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-xs hover:shadow-md transition-all duration-200">
                    <div 
                      className="px-5 py-4 cursor-pointer hover:bg-gray-50/50 flex flex-wrap items-center justify-between gap-4"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right hidden sm:block">
                          <p className="text-[11px] text-gray-400 font-bold uppercase tracking-wider">Items</p>
                          <p className="text-base font-black text-gray-700">{itemsCount}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[11px] text-gray-400 font-bold uppercase tracking-wider">Total</p>
                          <p className="text-lg font-black text-blue-600 leading-tight">৳{order.grandTotal.toLocaleString()}</p>
                        </div>
                        <div className={cn(
